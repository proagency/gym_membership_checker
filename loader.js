/*! Gym Membership Checker - single-file loader (scoped + gated) */
(function () {
  // -------- GATE: only run if the sub-account class is present --------
  var REQ_CLASS = "De4KzEafMS49YtrOrGeW";
  var hasClass = function () {
    return document.documentElement.classList.contains(REQ_CLASS) ||
           document.body.classList.contains(REQ_CLASS);
  };

  // If GHL is SPA-like and body class appears slightly later, wait briefly
  var start = function(){
    if (!hasClass()) return; // hard gate: do nothing if not the target sub-account
    ...
    // IMPORTANT: all selectors are scoped under #gymWidgetRoot
    var css = `
#gymWidgetRoot, #gymWidgetRoot * { font-family: 'Poppins', sans-serif; box-sizing: border-box; }
#gymWidgetRoot { all: initial; font-family: 'Poppins', sans-serif; } /* isolate defaults */
#gymWidgetRoot { position: relative; z-index: 99998; } /* base stacking context */

#gymWidgetRoot .overlay {
...
#gymWidgetRoot .card {
  ...
  overflow: hidden; animation: gymFadeInScale .25s ease;
}
#gymWidgetRoot .hd {
...
#gymWidgetRoot #manualInput, 
#gymWidgetRoot #branchInput {
  flex:1; border:1px solid #e5e7eb; border-radius:10px; padding:10px 12px; font-size:14px;
}
#gymWidgetRoot #lookupBtn, 
#gymWidgetRoot #saveBranchBtn {
...
#gymWidgetRoot #lookupBtn:hover, 
#gymWidgetRoot #saveBranchBtn:hover { filter: brightness(.95); }
#gymWidgetRoot #lookupBtn:disabled, 
#gymWidgetRoot #saveBranchBtn:disabled { background:#cbd5e1; cursor:not-allowed; }
#gymWidgetRoot .ghost-btn{
...
/* FAB is also inside the root so styling stays scoped */
#gymWidgetRoot .fab{
...
#gymWidgetRoot .fab:hover{ box-shadow:0 12px 28px rgba(0,0,0,0.12); transform: translateY(-2px); }
...
<div class="overlay" id="overlay">
  <div class="card" role="dialog" aria-modal="true" aria-labelledby="gym-ttl">
    ...
    var appendRoot = function(){ if (!document.getElementById("gymWidgetRoot")) document.body.appendChild(root); };
    if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", appendRoot); else appendRoot();

    // ----------------- JS LOGIC (unchanged except selectors are under root) -----------------
  ...
  if (document.readyState === "loading") {
    ...
    // small delay for SPA body classes to appear
    setTimeout(start, 50);
  }
})();
